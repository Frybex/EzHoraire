...
La réponse peut être gardée 2 minutes par un cache partagé (tous les
étudiants de la formation la partagent) et, si l'école tombe, la dernière
bonne version reste servie jusqu'à 7 jours (stale-if-error).

&frais=<n'importe quoi> : contourne les caches (bouton « Actualiser »).
...
CACHE_PARTAGE = "public, max-age=0, s-maxage=120, stale-while-revalidate=600, stale-if-error=604800"
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            repondre_json(self, 200, {"ok": True, "data": data}, "no-store" if frais else CACHE_PARTAGE)
